...
#### Sound Mode *(added in v0.3.5, rebuilt against real hardware in v0.3.7, quick-mode memory added in v0.3.8, Pure quick-select fixed in v0.3.9)*
...
- **Sound Mode (Quick)** -- Movie / Music / Game / Pure, mirroring the receiver's own remote/UI buttons. Sending one of these doesn't set a distinct state -- it jumps to whichever concrete mode was last selected within that category, leaving that mode untouched (confirmed: switching to Pure after Direct was active keeps it on Direct, it does not force a switch to Pure Direct). "Auto" behaves the same way one level down: it isn't a fixed state either, it resolves to whatever processing best fits current content (confirmed: selecting Auto with PCM content playing read back as "Dolby Audio - Dolby Surround", not "Auto"). Because of all this, this entity's shown state can only be determined for certain when the active mode uniquely belongs to one category (e.g. "Mono Movie" -> Movie, or "Direct"/"Pure Direct" -> Pure); for a mode reachable from more than one category (Stereo, Dolby Surround, DTS Neural:X, Multi Ch Stereo, Virtual -- reachable from Movie/Music/Game directly, or from Pure via Auto) the receiver genuinely doesn't expose which one produced it. To stay useful in that common case, the integration remembers which category you last picked (via this entity, or inferred whenever the receiver reports an unambiguous mode from *any* source -- its own remote, its app, or this integration) and keeps showing that until proven otherwise. This memory doesn't survive a Home Assistant restart, and can go stale if you switch categories entirely outside Home Assistant while landing on a mode reachable from more than one -- it only re-syncs the next time an unambiguous mode is observed.
