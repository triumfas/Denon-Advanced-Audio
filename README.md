...
#### Sound Mode *(added in v0.3.5, rebuilt against real hardware in v0.3.7, quick-mode memory added in v0.3.8, Pure quick-select fixed in v0.3.10)*

Both entities are set via telnet (`MS<mode>`). Their mode list and grouping were verified by probing an actual AVR-X3700H and cross-checking against its own "Sound Mode" UI, not guessed from protocol docs -- the receiver's UI groups modes into four categories: Movie, Music, Game, and Pure. Movie/Music/Game share a common pool of modes plus 1-3 exclusive to each; Pure has its own three (Direct, Pure Direct, Auto) -- though Auto itself behaves like a fifth "resolving" command (see below), so Pure isn't fully walled off from the shared pool either.

- **Sound Mode (Quick)** -- Movie / Music / Game / Pure, mirroring the receiver's own remote/UI buttons. For Movie/Music/Game, sending one of these doesn't set a distinct state -- it jumps to whichever concrete mode was last selected within that category, leaving that mode untouched. Pure has no equivalent telnet command (probing confirmed `MSPURE` is simply a no-op, not a real category-select), so the integration remembers which of Direct/Pure Direct/Auto you last used and sends that specific command instead -- defaulting to Pure Direct only if nothing's been remembered yet. "Auto" behaves like a fifth resolving command one level down: it isn't a fixed state either, it resolves to whatever processing best fits current content (confirmed: selecting Auto with PCM content playing read back as "Dolby Audio - Dolby Surround", not "Auto") -- picking it explicitly is the only way the integration can ever learn you use it, since it never shows up as the raw mode on its own. Because of all this, this entity's shown state can only be determined for certain when the active mode uniquely belongs to one category (e.g. "Mono Movie" -> Movie, or "Direct"/"Pure Direct" -> Pure); for a mode reachable from more than one category (Stereo, Dolby Surround, DTS Neural:X, Multi Ch Stereo, Virtual -- reachable from Movie/Music/Game directly, or from Pure via Auto) the receiver genuinely doesn't expose which one produced it, so the integration falls back to whichever category was last picked or inferred and keeps showing that until proven otherwise. Neither this memory nor the remembered Pure submode survive a Home Assistant restart, and both can go stale if you change modes entirely outside Home Assistant while landing on something reachable from more than one category -- they only re-sync the next time an unambiguous mode is observed.
- **Sound Mode** -- the full set of concretely-selectable modes (Stereo, Direct, Pure Direct, Auto, Dolby Surround, DTS Neural:X, Multi Ch Stereo, Virtual, Mono Movie, Rock Arena, Jazz Club, Matrix, Video Game). Hidden by default -- enable it for direct access. Its option list narrows to the modes belonging to the currently active category when that's determinable, or to the modes shared by Movie/Music/Game when it isn't (still ruling out Pure, whose three modes are never in that shared pool). "DTS Virtual:X" is recognized and displayed correctly when it's the receiver's active mode, but isn't offered as a choice -- no working SET command for it was found.
...
- **v0.3.10** -- v0.3.9's fix didn't hold up: `MSPURE` turned out to be a no-op rather than a real category-select command, so the Pure quick button still didn't reliably switch categories (it only appeared to work in the one case already tested, by coincidence). There's no telnet equivalent to `MSMOVIE`/`MSMUSIC`/`MSGAME` for Pure at all. Fixed properly this time: the integration now remembers which of Direct/Pure Direct/Auto you last used (updated whenever you pick one, and self-heals from Direct/Pure Direct whenever the receiver reports them from any source) and sends that specific command when you pick the Pure quick option, defaulting to Pure Direct only if nothing's been remembered yet. Verified end-to-end against real hardware: switching to Pure now preserves Direct instead of forcing Pure Direct, confirmed from a clean state and after switching through a different category first.
- **v0.3.9** -- Fixed **Sound Mode (Quick)**'s Pure button forcing a switch to "Pure Direct" every time instead of behaving like Movie/Music/Game (jump to category, leave the last-used mode inside it alone) -- it now sends the bare `MSPURE` category command instead of `MSPURE DIRECT`, confirmed by probing to preserve whatever was already active (e.g. stays on Direct instead of forcing Pure Direct). Also discovered "Auto" itself resolves to whatever processing fits current content rather than reading back as "Auto" (same behavior as Movie/Music/Game), so it's no longer treated as a reliable marker that Pure is active -- corrected the option-narrowing and remembered-category logic accordingly.
- **v0.3.8** -- **Sound Mode (Quick)** now remembers which category (Movie/Music/Game/Pure) you last picked, and keeps showing it while the active mode stays one shared across categories (Stereo, Dolby Surround, DTS Neural:X, Multi Ch Stereo, Virtual) instead of immediately reverting to unknown. The memory self-heals whenever the receiver reports an unambiguous mode, from any source (this integration, the receiver's own remote, or its app) -- it isn't persisted across a Home Assistant restart.
- **v0.3.7** -- Rebuilt the Sound Mode select entities' mode tables and grouping from scratch after finding real discrepancies against an actual AVR-X3700H: legacy names "Dolby Digital"/"DTS Surround"/"Standard"/"Left"/"Right" don't exist on this firmware -- the real modes are "Dolby Surround" (set via `STANDARD`) and "DTS Neural:X" (set via `DTS SURROUND`), confirmed to work regardless of source format (not gated by bitstream as previously assumed). Also discovered Movie/Music/Game/Pure are categories, not settable states -- sending one jumps to whatever mode was last active in that category, so the Quick select's state is only knowable when the active mode uniquely belongs to one category, and now correctly reports unknown otherwise instead of guessing. Removed the (incorrect) source-bitstream gating logic entirely.
