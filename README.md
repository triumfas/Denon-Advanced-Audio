...
- **Sound Mode (Quick)** -- Movie / Music / Game / Pure, mirroring the receiver's own remote/UI buttons. Sending one of these doesn't set a distinct state -- it jumps to whichever concrete mode was last selected within that category (that's genuine AVR behavior: the front-panel display doesn't show "MUSIC" either, it shows whatever specific mode that resolved to). Because of that, this entity's shown state can only be determined when the active mode uniquely belongs to one category (e.g. "Mono Movie" -> Movie); for a mode shared across Movie/Music/Game (Stereo, Dolby Surround, DTS Neural:X, Multi Ch Stereo, Virtual) the receiver genuinely doesn't expose which category is active, so it correctly shows as unknown rather than guessing.
