...
- **Sound Mode (Quick)** -- Movie / Music / Game / Pure, mirroring the receiver's own remote/UI buttons. Sending one of these doesn't set a distinct state -- it jumps to whichever concrete mode was last selected within that category (that's genuine AVR behavior: the front-panel display doesn't show "MUSIC" either, it shows whatever specific mode that resolved to). Because of that, this entity's shown state can only be determined for certain when the active mode uniquely belongs to one category (e.g. "Mono Movie" -> Movie); for a mode shared across Movie/Music/Game (Stereo, Dolby Surround, DTS Neural:X, Multi Ch Stereo, Virtual) the receiver genuinely doesn't expose which category is active. To stay useful in that common case, the integration remembers which category you last picked (via this entity, or inferred whenever the receiver reports an unambiguous mode from *any* source -- its own remote, its app, or this integration) and keeps showing that until proven otherwise. This memory doesn't survive a Home Assistant restart, and can go stale if you switch to a shared mode under a different category entirely outside Home Assistant -- it only re-syncs the next time an unambiguous mode is observed.
